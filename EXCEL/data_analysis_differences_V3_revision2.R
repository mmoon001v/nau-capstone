...
#write_xlsx(p_t5_excel_avg_comparisons, "p_t5_combined_avgs.xlsx")


# CSV aluminum ===============================================================
al_t5_bare_group_data <- t5_bare_group_data %>%
  filter(Element == "Al_mean") # modify
...
al_t5_excel_avg_comparisons <- al_t5_combined %>%
  group_by(Bin_Number, Inoculation, Surface,Date_Group) %>%
  summarise(
    mean_Al = mean(Value, na.rm = TRUE),
    sd_Al = sd(Value, na.rm = TRUE),
    n = n(),
    .groups = "drop")

#write_xlsx(al_t5_excel_avg_comparisons, "al_t5_combined_avgs.xlsx")



# CSV Potassium ================================================================
k_t5_bare_group_data <- t5_bare_group_data %>%
  filter(Element == "K_mean") # modify
k_t5_crust_group_data <- t5_crust_group_data %>%
  filter(Element == "K_mean") # modify

k_t5_combined <- bind_rows(
  k_t5_bare_group_data %>% mutate(Group = "Bare"),
  k_t5_crust_group_data %>% mutate(Group = "Crust")
)

k_t5_excel_avg_comparisons <- k_t5_combined %>%
  group_by(Bin_Number, Inoculation, Surface,Date_Group) %>%
  summarise(
    mean_K = mean(Value, na.rm = TRUE),
    sd_K = sd(Value, na.rm = TRUE),
    n = n(),
    .groups = "drop")

#write_xlsx(k_t5_excel_avg_comparisons, "k_t5_combined_avgs.xlsx")


# XLSX Iron ====================================
fe_t5_bare_group_data <- t5_bare_group_data %>%
  filter(Element == "Fe_mean") # modify
...
fe_t5_excel_avg_comparisons <- fe_t5_combined %>%
  group_by(Bin_Number, Inoculation, Surface,Date_Group) %>%
  summarise(
    mean_K = mean(Value, na.rm = TRUE),
    sd_K = sd(Value, na.rm = TRUE),
...
write_xlsx(fe_t5_excel_avg_comparisons, "fe_t5_combined_avgs.xlsx")
